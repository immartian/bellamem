 ...
 * Unlike Source.text (capped at MAX_TURN_CHARS=1500) and Source.toJSON()
 * (capped at 200 chars), this reads the original file and returns the
 * complete turn content.
 */
async function readFullTurn(filePath: string, turnIdx: number): Promise<string | null> {
...
export async function gatherEvidence(
  ...
  for (const sr of concept.sourceRefs) {
    ...
    // Read full text from the original jsonl.
    let fullText = "(source file not found)";
    if (src) {
      const read = await readFullTurn(src.filePath, src.turnIdx);
      fullText = read ?? src.text;  // fallback to the 1500-char in-memory copy
    }
