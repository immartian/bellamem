...
**Break-even: ~214 raw transcript tokens.**

Linear fit across 4 scenarios: `expand_tokens ≈ 106 + 0.51 × raw_tokens`.

Below ~200 raw tokens, Bella's per-belief metadata overhead dominates and the expand pack costs more tokens than the raw transcript would. Above the break-even point, Bella saves tokens, and the savings grow monotonically with dialogue size. Use Bella for conversations longer than ~200 tokens; for shorter chats, the flat tail is cheaper.
